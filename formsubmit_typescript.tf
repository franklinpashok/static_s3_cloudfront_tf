...
resource "aws_iam_role" "form_submit_test" {
  name        = "form-submit-role_typescript"
  description = "Role required by lambda function to trigger email for typescript lambda fucntion"

  assume_role_policy = data.aws_iam_policy_document.lambda-assume-role-policy.json

  inline_policy {
    name = "form-submission-policy_typescript"
...
resource "aws_lambda_function" "form_submit_lambda" {
  ...
  function_name = "form_submit_lambda_function_typescript"
